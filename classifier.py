...
images = np.load('data/poke_image_data.npy')
images = images / 255
n, width, length, color_num = images.shape

labels = pd.read_csv('data/names_and_strengths.csv')
...
model = Sequential()  
model.add(Conv2D(32, (5, 5), strides = 3, input_shape = (width, length, color_num), activation = 'relu'))
...
model.add(Dense(output_dim = kinds, activation = 'sigmoid'))

model.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])
model.summary()

model.fit_generator(generator=train_imgen.flow(train_x, train_y, batch_size=64), 
                    validation_data=test_imgen.flow(test_x, test_y, batch_size=64), 
